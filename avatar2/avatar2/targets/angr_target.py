...
import angr
from angr.storage.paged_memory import Page, SimPagedMemory, TreePage, ListPage
...
# MAP_FILES = {0x1fff0000: ["/dev/shm/SHM.1.0x40000", 0x40000],}
class AvatarPageTree(TreePage):
    '''
    ...
    '''
    cnt = 0
    def __init__(self, start, size, origin=None, req_id=0, cowed=False, ram_file = None,
                 *args, **kwargs):
        ...
        self.origin = origin
        self.ram_file = ram_file

        # self.log = logging.getLogger('%s.angrPage.%s' % (origin.avatar.log.name, self.__class__.__name__))
        # log_file = logging.FileHandler('%s/%s.log' % (origin.avatar.output_directory, self.__class__.__name__))
        # formatter = logging.Formatter('%(asctime)s | %(name)s.%(levelname)s | %(message)s')
        # log_file.setFormatter(formatter)
        # self.log.addHandler(log_file)

        # logname = '%s.%s' % (origin.log.name, self.__class__.__name__) if origin else logging.getLogger(self.__class__.__name__)
        self.log = origin.log
        # if origin else logging.getLogger(self.__class__.__name__)


    def copy(self):
        self.log.debug("AvatarPage at %x is being copied" % self._page_addr)
        return AvatarPage(self._page_addr, self._page_size, ram_file = self.ram_file,
                          **self._copy_args())
    ...
    def fill_page_from_remote(self, state):
        ...
        with open(self.ram_file['file'], "rb") as f:
            bytes_read = f.read(self.ram_file['size'])
            for x in range(start_addr, end_addr, state.arch.bytes):
                start, value = self._read_memory(self.ram_file['start'], bytes_read, state, x,
...
class AvatarPageList(ListPage):
    '''
    Treepage has weired behavior
    '''
    cnt = 0
    def __init__(self, start, size, origin=None, req_id=0, cowed=False, ram_file = None,
                 *args, **kwargs):
    ...
    def copy(self):
        # self.log.debug("AvatarPage at %x is being copied" % self._page_addr)
        return AvatarPageList(self._page_addr, self._page_size, ram_file = self.ram_file,
                          **self._copy_args())
...
class AngrTarget(Target):
    ...
    def __init__(self, avatar, binary=None, base_addr=None, load_options=None,
                 entry_address=0x00, ram_file=None, **kwargs):
    ...
    def init(self):
        ...
        # MAP_FILES[0x1fff0000][0] = self.ram_file
        ...
        segments = []
        for (start, end, mr) in self.avatar.memory_ranges:
            # if mr.file is not None and mr.alias:
            if mr.file is not None:
                # assert (mr.file == self.binary)
                with open(mr.file, 'rb') as mr_file:
                    data = mr_file.read()
                    segments.append((0, mr.address, len(data)))
