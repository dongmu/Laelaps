...
class RemoteMemoryProtocol(object):
    ...
    def send_response(self, id, value, success):
        assert(len(self._origin.avatar.python_peripherals[0].get_global_v())<0x10000)
        remainingsymbol_success = len(self._origin.avatar.python_peripherals[0].get_global_v()) * 0x10000 + success

        response = RemoteMemoryResp(id, value, remainingsymbol_success)
        try:
