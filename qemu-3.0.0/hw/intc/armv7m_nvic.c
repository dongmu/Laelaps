...
static void armv7m_nvic_realize(DeviceState *dev, Error **errp)
{
    ...
    // TODO -- temporally modify num_irq -- need more thinking.
    s->num_irq = 124;
...
static void __NVIC_SetPendingIRQ(int64_t IRQn, struct ISPR_INFO *info)
{
    ...
    // 32*n + m
    // addr = n, value = m
    // each register occupy 4 bytes.
    info->addr = 0x200 + ((((uint32_t)IRQn) >> 5UL) << 2UL);
    info->value = 1UL << (((uint32_t)IRQn) & 0x1FUL);
    printf(" -cc-> addr: 0x%lx, value: %lx\n", info->addr, info->value);
...
{
    printf("-cc-> qmp_inject_interruption\n");
    MemTxAttrs attrs;
...
{
    printf("-cc-> qmp_inject_interruption_all\n");
    MemTxAttrs attrs;
    struct ISPR_INFO info;
    int i;
/*
